...
export default function Home() {
    ...
    return (
        <main className="flex min-h-screen flex-col items-center justify-center gap-6 p-24 bg-background text-foreground relative">
            {isLocked && (
                // biome-ignore lint/a11y/useKeyWithClickEvents: This click is just producing a "blocking" visual effect
                <main
                    className="fixed inset-0 z-50 bg-transparent cursor-default"
                    onClick={() => {
                        const win = getCurrentWindow();
                        win.emit("request-shake", {
                            windowLabel: win.label,
                        }).then((_) => Logger.info("Emitted request-shake event", "Home"));
                    }}
                />
            )}
